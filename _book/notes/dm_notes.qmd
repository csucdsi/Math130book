...
```{r}
library(_______)                    # this came installed with tidyverse
library(_______)                  # for fancy summary tables
...
```{r}
_______(ncbirths$lowbirthweight)
```
...
The variable `ncbirths$lowbirthweight` has _______ records with a value of `low`, and _______ records with the value of `not low`.
...
```{r}
_______ <- _______[_______(ncbirths$premie), ]
ggplot(_______, aes(premie)) + geom_bar()
```
...
```{r}
ncbirths$_______[ncbirths$_______] <- _______
```
...
```{r}
ncbirths$_______ <- _______(ncbirths$_______, "_______", "_______")
```
...
{.callout-tip title = "👉Example: Frequency tables & summary statistics using the pipe

First stating the variable, then pipe in the summary function.

```{r}
ncbirths$mature |> _______() # instead of table(ncbirths$mature)
ncbirths$mage |> _______() #instead of mean(ncbirths$mage)
```
