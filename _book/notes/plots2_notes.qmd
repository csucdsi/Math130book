...
ggplot(pen, aes(x=body_mass_g)) + 
  geom_histogram() + 
  _________(aes(_________=_________(_________, na.rm = TRUE)),
            color="blue", linetype="dashed")

ggplot(pen, aes(y=body_mass_g, x = island)) + 
  geom_boxplot() + 
  _________(fun="_________", geom="_________", 
               shape=_________, size=_________) 
```
...
NCbirths %>%   
  ...
  filter(!is.na(habit)) %>% 
ggplot(aes(x=_________, y=_________, color = _________)) + 
         geom_point() + geom_smooth(se=FALSE) + 
  theme_minimal() + 
  _________(palette = "_________") 
```
