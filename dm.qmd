...
2. Right click and "save as" this lessons [[04_dm_notes.qmd]](04_dm_notes.qmd) Quarto notes file and save into your `notes/Math130` folder. 
3. In the _Files_ pane, open this Quarto file and Render this file.   
:::
...
Numerical variables can be summarized using quantities called _summary statistics_ which include the `min`, `max`, `mean` and median. The function `summary()` prints out the five number summary, and includes the mean. This function also displays the number of missing values for that variable. 
...
The first argument is the data set, then you `include` the vector of variable that you want to display in the table. By default the sample size _n_ and the relative percent are presented for categorical data, and the median, with first and third quartiles shown for quantitiatve data. 
...
Sometimes the value for a variable is missing. Think of it as a blank cell in an spreadsheet. Missing data can be a result of many things: skip patterns in a survey (i.e. non-smokers don't get asked how many packs per week they smoke), errors in data reads from a machine, etc. 
...
**Example problem 1: `R` can't do arithmetic on missing data.** 
...
**Example Problem 2: Some plots will show `NA` as it's own category**

Sometimes this is fine, other times this is undesirable. We'll see later how we can fix this problem 
...
We can look at the raw data using `head()` or opening the data set in the spreadsheet view. This may not be helpful if there is no missing values in the first 6 rows, or if there is a large number of variables to look through. 
...
Sometimes you want to operate only only the non-missing values. Recall from @sec-logical we can use the `!` to negate a boolean argument. We can use this tactic to fix that barchart from above.
...
Wrap `!is.na()` around the `premie` variable in the `ggplot` code to create a barchart that does not have a bar for missing values. 
...
Sometimes we have a need to edit, or change data. This is also known as **recoding**. One way to change values of existing data is by using logical statements that you learned earlier. 

To choose all observations (rows) of a `data` set, where a `variable` is equal to some `value`, then set assign `<-` a `new value` to those rows. 
```{r}
#| eval: false
data[data$variable==value] <- new_value  # example code. do not run
```
...
2. _and then_ create a frequency table on that variable

and 

1. Get the `mage` variable from the `ncbirths` data set 
2. _and then_ calculate the mean of that variable
