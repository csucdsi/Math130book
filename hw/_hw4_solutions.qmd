...
c. Group by school type. Check for a different relationship between math and social studies scores depending on school type. Color the points and lines by the `schtyp` variable. _hint: you're **not** adding code to the `geom` layers_
...
a. Use `dplyr` verbs to 1) Group the data by `state`, and then, 2) calculate the average percentage of Hispanic-owned firms (`hispanic_owned_firms`) within each state, and the standard error of this average using `se = number of counties used to calculate each state’s average. (_hint: these can be done in the same `mutate` statement with the function `n()`_)
