...
```{r}
ff$restaurant <- ________(ff$restaurant)
________(ff$restaurant)
```
...
```{r}
table(________) # view which values are present
________(mtcars$am) # confirm what R thinks the data type is
```
...
```{r}
mtcars$________ <- factor(mtcars$am, 
                                   labels=c("________", "________"))
```
...
```{r}
table(mtcars$________, mtcars$________, useNA="always")
```
...
```{r}
email$________ <- factor(email$________, 
                              labels=c( "None", "<1M","1M+"))
table(email$________, email$________, useNA="always")
```
...
factor(ff$restaurant, 
       levels=c("________", "________", "________", "Mcdonalds", "Sonic", 
...
# results not saved
ff$restaurant |> 
  fct_relevel("________", "________", "Dairy Queen", "Mcdonalds", "Sonic") |> 
...
```{r}
ff$restaurant |> 
  fct_relevel("Subway", ________) |> # move to end
...
ff$restaurant_new <- fct_collapse(ff$restaurant, 
                                    ________ = c("Burger King", "Mcdonalds", "Sonic"), 
                                    ________ = c("Arbys", "Subway"))
...
```{r}
ff$restaurant[ff$________ == "Dairy Queen"] <- ________
table(________)
```
