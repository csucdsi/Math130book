...
We will see how to use these logical statements to do things such as subsetting data and creating new variables in a few sections.  
...
```{r}
weight_KG ____  # Returns TRUE or FALSE for each element in the vector
```
...
```{r}
weight_KG ______  # returns TRUE or FALSE depending on which elements of the vector are greater than 1
```
...
```{r}
______[weight_KG < 1 ______ weight_KG > 5]
```
...
```{r}
______[weight_KG >= .1 & weight_KG <= 5]
```

A common task is to search for certain strings in a vector.  One could use the "or" operator `|` to test for equality to multiple values, but this can quickly become tedious. The function `%in%` allows you to test if any of the elements of a search vector are found:

```{r}
animals[animals ______ | animals ______] # returns both rat and cat
animals ______ c("rat", "cat", "dog", "duck", "goat")
______[______ %in% c("rat", "cat", "dog", "duck", "goat")]
```
...
```{r}
mean(diamonds$______[diamonds$______])
```
