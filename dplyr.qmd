...
# Efficient data management {#sec-dplyr}

[**Efficient data management** video](https://media.csuchico.edu/media/MATH+130+-+Section+7+Intro/1_9w8ieaku)
...
2.  Right click and "save as" this lessons [[Quarto notes file]](notes/dplyr_notes.qmd) and save into your `Math130/notes` folder.
...
[**Tibbles** video](https://media.csuchico.edu/media/MATH+130+-+Section+7.1/1_ae7p85p6)
...
[**Filter** video](https://media.csuchico.edu/media/MATH+130+-+Section+7.2.1/1_xcn8y6j7)
...
[**Select** video](https://media.csuchico.edu/media/MATH+130+-+Section+7.2.2/1_mtbbgy5p)
...
[**Rename** video](https://media.csuchico.edu/media/MATH+130+-+Section+7.2.3/1_jy59mokz)
...
Sometimes variables come to you in really obscure naming conventions. What the heck is SBA641? The `rename()` function converts an `old name` to a `new name`. The generic syntax is `rename(new = old)`. [For the purpose of these lecture notes I am not making this change permanent. There is no assignment operator `<-` used here, so this change is not going to persist into later code.]{.aside}
...
[**Mutate** video](https://media.csuchico.edu/media/MATH+130+-+Section+7.2.4/1_m02kzrh7)
...
[**Summarize** video](https://media.csuchico.edu/media/MATH+130+-+Section+7.2.5/1_f3xyklec)

Next up is the verb `summarise()`, which is most often used to calculate summary statistics. 
```{r}
summarise(flights, 
...
[**Arrange** video](https://media.csuchico.edu/media/MATH+130+-+Section+7.2.6/1_h0yaxknf)
...
::: {.column width="50%"}
To reverse the sort order, use `desc()`.  This is Descending order. 
...
:::{.callout-important title = "Avoid the base `sort` function"}
There is a `sort` and `order` functions in base R, but they operate on a single vector only. They do _not_ keep all rows in a data frame together when sorting rows, so avoid using it. 
:::

### Relocate

[**Relocate** video](https://media.csuchico.edu/media/MATH+130+-+Section+7.2.7/1_dbxs6uh0)
...
[**Case when** video](https://media.csuchico.edu/media/MATH+130+-+Section+7.2.8/1_jhqy0au6)

![Cartoon showing a table with creature type (kraken, dragon, or cyclops) and age (baby, teen, or adult). The three creatures listed are adding a new column named “danger”, which contains the word “extreme!” if the type is “kraken”, or “high” for any other type. Stylized text reads “dplyr::case_when() - IF ELSE...but you love it? An example of code is shown: mutate(danger = case_when(type == “kraken” ~ “extreme!”, TRUE ~ “high”).](img/case_when.png)

[Learn more about [case_when](https://dplyr.tidyverse.org/reference/case_when.html)]{.aside}

This function allows you to vectorise multiple `if_else()` statements. Each case is evaluated sequentially and the first match for each element determines the corresponding value in the output vector. If no cases match, the `.default` is used as a final "else" statement.
...
This `case_when` function can be used with complex logical statements involving multiple variables. Additionally, you don't have to specify every single possible combination. If none of the statements specified evaluate as TRUE, the `.default` value will be used. 
...
3. If neither of these two statements are true, there is a delay at either the departure or arrival and so we set `delay_type` to "one delay". 


## Grouped Operations  

[**Grouped Operations** video](https://media.csuchico.edu/media/MATH+130+-+Section+7.3/1_007m0j9k)
...
```{r}
summarise(by_month, avg_distance = mean(distance, na.rm=TRUE))
```
...
Or even more interestingly, the average departure delay, number of flights with delays, and the maximum length of delay at each of the three origin airports. 
...
[**Chaining Operations** video](https://media.csuchico.edu/media/MATH+130+-+Section+7.4/1_gfoztd73)
