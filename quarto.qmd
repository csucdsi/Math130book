...
:::{.callout-tip title = "👉 Open the Quarto tutorial file" icon=false}
...
:::{.callout-tip title = "👉 Add your name" icon=false}
...
* R code chunks can also have a YAML style header, identified by `#|` at the beginning of the line. In this case I set `eval` to `false` to prevent the code inside this code chunk from being run when I render the whole document. 
...
:::{.callout-tip title = "👉 Make a change" icon=false}
...
2.  Type `colors()` in your console to pull up all the named colors in R. Change at least one color in the penguin plot. 
...
:::{.callout-tip title = "👉 Add some code" icon=false}
...
[See more basic syntax of Markdown at <a href="https://quarto.org/docs/authoring/markdown-basics.html"> https://quarto.org/docs/authoring/markdown-basics.html]{.aside}
...
:::{.callout-tip title = "👉 Add some formatting" icon=false}
...
Sometimes you want to hide your code, control the size and placement of figures, or not run a block of code at all. We can control these at the individual code chunk level using code chunk options. These are prefixed by a "hash-pipe" `#|` at the top of the code chunk.[`|` is the first of three pipes we'll see in this course]{.aside} 
[See more options in the [Quarto cocumentation](https://quarto.org/docs/computations/execution-options.html)]{.aside}

If we set `eval: false` it will not render any code in that code chunk. Great for when you have an error but not sure how to fix it yet, but you want to render your full document. 
```{r}
#| echo: fenced
#| eval: false
2+2 # not evaluated
...
Setting `echo: false` hides the code chunk from being shown, but shows the results. 
```{r}
#| echo: false
2+2 
```

We will see more uses of code chunk options when we get to multiple plots from one code chunk. 
