...
The **console** is where you see the execution of commands.  
You can type commands directly into the console and press
<kbd>Enter</kbd> to execute those commands, but they will be forgotten when you close the session.
...
:::{.callout-tip title = "👉 Your Turn" icon=false}
Now try a more complicated equation. Type this in _exactly_ as it is written below. 
```{r}
#| error: true
2 + 5*(8^3)- 3*log10)
```
...
::: {.callout-caution icon=false title = "Navigating R Studio"}
Optional: Watch one of the following short videos to learn how to navigate R Studio.
