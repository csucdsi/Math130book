...
R projects are a great way of keeping all files for one project all together, and makes importing data much easier by using relative paths instead of absolute ones. Plus this ensures reproducibility by others (because no-one else has a path at `C:\users\rdonatello\myprojects\math130`)
...
* Using the `files` tab (bottom right) open the `_hello.qmd` script files.
:::
